...
class TestSQLTable:
    ...
    def test_lambda_column(self, tbl1, tbl2):
        ...
        # Join
        assert_equal(
            tbl1 >> select() >> mutate(a = tbl1.col1 * 2) >> join(tbl2, λ.a == tbl2.col1, 'left'),
            tbl1 >> select() >> mutate(a = tbl1.col1 * 2) >> join(tbl2, tbl1.col1 * 2 == tbl2.col1, 'left')
        )
