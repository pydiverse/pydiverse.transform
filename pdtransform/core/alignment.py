...
def aligned(*, with_: str):
    ...
    def decorator(func):
        ...
        if not isinstance(with_, str):
            raise Exception(f"Argument 'with_' must be of type str, not '{type(with_).__name__}'.")
        ...
        def wrapper(*args, **kwargs):
            ...
            if not isinstance(result, symbolic_expressions.SymbolicExpression):
                raise ValueError(f"Aligned function must return a symbolic expression not '{result}'.")
            ...
            elif isinstance(alignment_param, (table.Table, table_impl.AbstractTableImpl)):
...
def eval_aligned(
        ...
        **kwargs) -> 'symbolic_expressions.SymbolicExpression[column.LiteralColumn]':
    ...
    if with_ is not None:
        ...
        if not isinstance(with_, table_impl.AbstractTableImpl):
            raise ValueError(f"'with_' must either be an instance of a Table or TableImpl. Not '{with_}'.")
